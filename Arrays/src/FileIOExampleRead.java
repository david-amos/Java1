...
public class FileIOExampleRead {
    ...
    static String[] arrDeptNames = {"Marketing", "Sales", "Accounting", "Support", "IT", "Janitorial", "QA"};
    //Initializer list
    ...
    public static void init(){
        ...
        pw.format("%-25s%5s%-25s%5s%-10s%5s%5s%5s%5s%5s%10s%n%n", "First Name", " ", "Last Name", " ", "Department"," ",
                "Hours", " ", "Rate", " ", "Pay Amount");
        input();
    }
    ...
    public static void output(){
        int deptIndex;
        for(deptIndex = 0;deptIndex < arrDeptNumbers.length;deptIndex++){
            if(iDept == arrDeptNumbers[deptIndex]){
                break;
            }
        }
        pw.format("%-25s%5s%-25s%5s%-10s%5s%5d%5s%5.2f%5s%10.2f%n%n", iFirstName, " ", iLastName, " ", arrDeptNames[deptIndex], " ",
                iHours, " ", iRate, " ", cPay);
    }
